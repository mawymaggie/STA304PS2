...
The objective of this model is to predict the result of the 2020 American federal election. We achieve this by constructing a logistic regression model using 20200625 survey data, then we use post stratification technique on census data to extrapolate the population estimate. 
...
```{r,echo=FALSE}
kable(summary(model)$coefficients, caption = "Table 1 - Summary of Logistic Model", digits = 5)
```

```{r, echo=FALSE}

bar_1 <- ggplot(data = survey_data, aes(x =survey_data$race, fill = survey_data$vote_2020)) +
  geom_bar() + ## Create a bar chart.
  theme(text = element_text(size = 6)) + ## Adjust the size of labels.
  labs(x = "Race",
       y = "Number",
       fill = "Voter's Choice",
       title = "Bar Plot for Voter's Choice and Race") ## Add labels for x axis and y axis, and add title.

bar_2 <- ggplot(data = survey_data, aes(x = survey_data$age_group, fill = survey_data$vote_2020)) +
  geom_bar() + ## Create a bar chart.
  theme(text = element_text(size = 6)) + ## Adjust the size of labels.
  labs(x = "Age Group",
       y = "Number",
       fill = "Voter's Choice",
       title = "Bar Plot for Voter's Choice and Age Group") ## Add labels for x axis and y axis, and add title.

bar_3 <- ggplot(data = survey_data, aes(x = survey_data$state, fill = survey_data$vote_2020)) +
  geom_bar() + ## Create a bar chart.
  theme(text = element_text(size = 6)) + ## Adjust the size of labels.
  labs(x = "State",
       y = "Number",
       fill = "Voter's Choice",
       title = "Bar Plot for Voter's Choice and State") ## Add labels for x axis and y axis, and add title.

bar_4 <- ggplot(data = survey_data, aes(x = survey_data$household_income, fill = survey_data$vote_2020)) +
  geom_bar() + ## Create a bar chart.
  theme(text = element_text(size = 6)) + ## Adjust the size of labels.
  labs(x = "Household Income",
       y = "Number",
       fill = "Voter's Choice",
       title = "Bar Plot for Voter's Choice and Household") ## Add labels for x axis and y axis, and add title.

bar_5 <- ggplot(data = survey_data, aes(x = survey_data$education, fill = survey_data$vote_2020)) +
  geom_bar() + ## Create a bar chart.
  theme(text = element_text(size = 6)) + ## Adjust the size of labels.
  labs(x = "Education",
       y = "Number",
       fill = "Voter's Choice",
       title = "Bar Plot for Voter's Choice and Education") ## Add labels for x axis and y axis, and add title.

ggarrange(bar_1, bar_2, bar_3, bar_4, bar_5, ncol = 2, nrow = 3)
...
boxplot_6 <- ggplot(census_data, aes(x='', y=census_data$estimate)) +
  geom_boxplot(aes(fill=census_data$race)) +
  labs(x = "",
       y = "estimate",
       fill = "Race",
       title = "Boxplot for estimate and Race") ## Add labels for x axis and y axis, and add title.
boxplot_6
```
From the boxplot we obtain that the mean percentage that the race of voter is black vote Donald Trump is the lowest, and the mean percentage that the race of voter is American native vote Donald Trump is the highest. And there are some outliers in the boxplot. The percentage that the race of voter is black vote Donald Trump are more concentrate. All the estimate is bigger than 0.50.
